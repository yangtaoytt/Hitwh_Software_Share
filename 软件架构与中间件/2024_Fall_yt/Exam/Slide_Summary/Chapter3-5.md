...
Swarm的调度策略：（1）random策略：随机选择节点。一般用于开发测试阶段。（2） spread策略：默认策略，swarm优先选择占用资源（如CPU、内存等）最少的节点，能保证集群中所有节点资源的均匀使用。（3）binpack策略：与spread相反，它的目的是尽可能地填满一个节点，以保证更多空余的节点。
